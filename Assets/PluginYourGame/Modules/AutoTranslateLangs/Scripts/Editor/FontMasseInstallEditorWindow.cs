...
namespace YG.LanguageLegacy
{
    ...
    {
        [MenuItem("YG2/Auto Translate Langs/Font Default Masse")]
            ...
            if (objectsTranlate.Count > 0)
            {
                ...
                if (GUILayout.Button("Put a standard font on all selected texts", GUILayout.Height(30)))
                {
                    ...
                    foreach (GameObject obj in objectsTranlate)
                    {
                        ...
                        if (scr.info.fonts.defaultFont.Length >= scr.fontNumber + 1 && scr.info.fonts.defaultFont[scr.fontNumber])
                        {
                            if (scr.textLComponent)
                                scr.ChangeFont(scr.info.fonts.defaultFont);
#if TMP_YG2
                            else if (scr.textMPComponent)
                                scr.ChangeFont(scr.info.fontsTMP.defaultFont);
#endif
                            completeObjCoint++;
                        }
                        else
